...
func SeedWorkoutData() []WorkoutComponent {
	...
	// Create metadata
	metadata1 := NewItemMetadata("001", time.Now(), &title1)
	metadata2 := NewItemMetadata("002", time.Now(), &title2)
	metadata3 := NewItemMetadata("003", time.Now(), &title3)
	metadata4 := NewItemMetadata("004", time.Now(), &title4)
	metadata5 := NewItemMetadata("005", time.Now(), &title5)

	// Define example RepComponents
	repComponent1 := RepComponent{
	...
	nonRepComponent1 := NonRepComponent{Duration: 30} // 30-minute cardio

	nonRepComponent2 := NonRepComponent{Duration: 45} // 45-minute cardio
